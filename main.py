...
parser.add_argument("-i", "--input", help="Video path, stream URI, or camera ID ", default="demo.mp4")
parser.add_argument("-t", "--threshold", type=float, default=0.3, help="Minimum score to consider")
parser.add_argument("-m", "--mode", choices=['detection', 'tracking'], default="tracking", help="Either detection or tracking mode")
...
def illustrate_box(image: np.ndarray, box: np.ndarray, caption: str) -> None:
    ...
    cv2.putText(image, caption, tuple(p1), cv2.FONT_HERSHEY_SIMPLEX, 0.75, TEXT_COLOR, 2)
...
def illustrate_tracking_info(frame: np.ndarray) -> np.ndarray:
    for num, (class_id, tracker) in enumerate(trackers.items()):
        ...
        cv2.putText(frame, txt, (0, 50 * (num + 1)), cv2.FONT_HERSHEY_SIMPLEX, 0.75, TEXT_COLOR, 2)
    return frame

trackers = {CLASSES_90.index(tracked_class): Sort() for tracked_class in TRACKED_CLASSES}

def track(dets: np.ndarray, illustration_frame: np.ndarray = None) -> np.ndarray:
    for class_id, tracker in trackers.items():
        ...
        if illustration_frame is not None:
            for box in sort_boxes:
                illustrate_box(illustration_frame, box[:4], f"{CLASSES_90[class_id]} {int(box[4])}")
    return illustration_frame

cap = cv2.VideoCapture(args.input)

for res, frame in iter(cap.read, (False, None)):
    detector.setInput(cv2.dnn.blobFromImage(frame, size=INPUT_SIZE, swapRB=True, crop=False))
